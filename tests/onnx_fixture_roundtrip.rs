...
#[test]
#[ignore]
fn ecdc_chunk_matrix_verbose() -> Result<()> {
    let bundle_dir = env::var("ENCODEC_RS_MATRIX_BUNDLE")
        .map(PathBuf::from)
        .unwrap_or_else(|_| PathBuf::from("onnx-bundles/encodec_48khz_6kbps"));
    ...
    if !bundle_dir.join("bundle.json").exists() {
        eprintln!(
            "skipping ecdc_chunk_matrix_verbose because {} does not exist",
    ...
    let cases = [
        ...
        Case {
            label: "1333.3ms",
            chunk_ms: Some(1333.3),
        },
        Case {
            label: "1800.0ms",
            chunk_ms: Some(1800.0),
        },
    ];
    ...
    for case in cases {
        ...
        let mut codec = OnnxFrameCodec::from_dir(&bundle_dir, execution_target(&bundle_dir)?)?;
        ...
        let mut lm_codec = OnnxLmCodec::from_dir(&bundle_dir, execution_target(&bundle_dir)?)?;
        ...
        println!(
            "encoded case={} parts={} bytes={} encode_s={:.6} audio_s={:.6} rtf={:.6}",
            case.label,
            emitted_parts,
            ecdc_bytes.len(),
            encode_seconds,
        ...
        let inspected = inspect_ecdc_chunks(case.label, case.chunk_ms, &ecdc_bytes, &meta)?;
        summaries.push(SummaryRow {
            label: case.label,
            chunk_ms: case.chunk_ms,
    ...
    for row in summaries {
        println!(
            "{:<12} {:>12} {:>8} {:>12.6} {:>12.6} {:>12.1} {:>16} {:>16} {:>16} {:>12}",
            row.label,
            format_chunk_ms(row.chunk_ms),
            row.chunks,
            row.encode_seconds,
...
fn inspect_ecdc_chunks(
    ...
    chunk_ms: Option<f64>,
    ecdc_bytes: &[u8],
    meta: &encodec_rs::metadata::OnnxFrameBundleMetadata,
    ...
    if chunk_samples == 0 {
        bail!("case {} has invalid chunk_samples=0", label);
    }
    if chunk_stride == 0 {
        bail!("case {} has invalid chunk_stride=0", label);
    }

    println!(
        "metadata case={} requested_chunk_ms={} audio_length={} sample_rate={} chunk_samples={} chunk_stride={} chunk_duration_ms={:.6} header_bytes={}",
        label,
        format_chunk_ms(chunk_ms),
    ...
    for (index, offset) in starts.iter().copied().enumerate() {
        ...
        let payload = read_chunk_payload(&mut reader, true)
            .with_context(|| format!("failed to read chunk {} for case {}", index, label))?;
