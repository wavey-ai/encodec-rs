...
print(str(int(value)) if value.is_integer() else str(value).replace(".", "p"))
PY
  )"
...
if num_codebooks > int(lm.n_q):
    raise SystemExit(
        f"LM only has {lm.n_q} codebooks, requested {num_codebooks}"
    )


def tensor_to_f32_bytes(tensor: torch.Tensor) -> bytes:
    array = (
        tensor.detach()
        .cpu()
        .to(torch.float32)
...
def quantized_linear_bytes(
    ...
    if array.ndim != 2:
        raise SystemExit(
            f"expected 2D weight, got {array.shape}"
        )
    ...
    quantized = (
        np.round(array / scales[:, None])
        .clip(-127, 127)
        .astype(np.int8)
    )

    return scales.tobytes(), quantized.tobytes()


def write_f32_vec(out, tensor: torch.Tensor) -> None:
    out.write(
        tensor_to_f32_bytes(tensor.reshape(-1))
    )


def write_quantized_linear(
    out,
    linear_or_weight,
) -> None:
    weight = (
        linear_or_weight.weight
        if hasattr(linear_or_weight, "weight")
        else linear_or_weight
    )

    scales, quantized = quantized_linear_bytes(weight)
    out.write(scales)
    out.write(quantized)
...
def transformer_norm_in(
    ...
    for name in (
        ...
        if (
            module is not None
            and hasattr(module, "weight")
            and module.weight.numel() == fallback_dim
        ):
            return norm_pair(
                module,
                fallback_dim,
...
def call_create_sin_embedding(
    ...
    if "max_period" in signature.parameters:
...
def fallback_sin_embedding(
    ...
    if dim % 2 != 0:
        raise SystemExit(
            "fallback sinusoidal embedding requires even dim"
        )
...
def positional_embedding(
    ...
    try:
        positions = torch.arange(
            frame_length,
            dtype=torch.float64,
        ).view(1, frame_length, 1)

        position = call_create_sin_embedding(
            positions,
            dim,
            max_period,
        )
        ...
        position = fallback_sin_embedding(
            frame_length,
            dim,
            max_period,
        )
...
def write_layer(out, layer) -> None:
    attention = layer_attr(
        layer,
        "self_attn",
    )

    write_quantized_linear(
        out,
...
norm_weight, norm_bias = transformer_norm_in(
    lm.transformer,
    dim,
)
...
if position.shape != (
    ...
    raise SystemExit(
        f"positional embedding shape {tuple(position.shape)} "
        f"does not match {(frame_length, dim)}"
    )
    ...
    json.dumps(
        {
            ...
            "bandwidth_kbps": bundle.get(
                "bandwidth_kbps"
            ),
            "segment_samples": bundle.get(
                "segment_samples"
            ),
            "segment_stride": bundle.get(
                "segment_stride"
            ),
            "num_codebooks": bundle.get(
                "num_codebooks"
            ),
            "frame_length": bundle.get(
                "frame_length"
            ),
            "lm_quant_weight_model": bundle.get(
                "lm_quant_weight_model"
            ),
            "lm_dim": bundle.get(
                "lm_dim"
            ),
            "lm_num_layers": bundle.get(
                "lm_num_layers"
            ),
            "lm_past_context": bundle.get(
                "lm_past_context"
            ),
            "lm_cardinality": bundle.get(
                "lm_cardinality"
            ),
        },
        indent=2,
...
if bundle.get("segment_samples") != trace_samples:
    raise SystemExit(
        f"segment_samples {bundle.get('segment_samples')} "
        f"!= trace_samples {trace_samples}"
    )

if bundle.get("segment_stride") != trace_stride:
    raise SystemExit(
        f"segment_stride {bundle.get('segment_stride')} "
